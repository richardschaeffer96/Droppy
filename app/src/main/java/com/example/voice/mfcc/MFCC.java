...
public class MFCC {
	...
	public MFCC(int n_mfcc,int n_mels,float fMin,int n_fft,int hop_length,float sampleRate,float fMax) {
		this.n_mfcc=n_mfcc;
		this.fMin=fMin;
		this.fMax=fMax;
		this.n_fft=n_fft;
	...
	private float    fMax                 = sampleRate/2.0f;

	FFT fft = new FFT();


	public float[][]output(double[]input) {
		return dctMfcc(input);
	}

	public float[][] process(double[] doubleInputBuffer) {
		return dctMfcc(doubleInputBuffer);
...
//mel spectrogram, librosa
	public float[][] melSpectrogram(double[] y){
...
//stft, librosa
	private float[][] stftMagSpec(double[] y){
		...
		for (int i = 0; i < n_fft/2; i++){
			ypad[(n_fft/2)-i-1] = (float)y[i+1];
			ypad[(n_fft/2)+y.length+i] = (float)y[y.length-2-i];
		}
		for (int j = 0; j < y.length; j++){
			ypad[(n_fft/2)+j] = (float)y[j];
		}
	...
	private float[] magSpectrogram(double[] frame){
	  ...
	  final float logstep = (float)(Math.log(6.4) / 27.0);              // step size for log region
		...
		for (int i = 0; i < freqs.length; i++) {
			if (freqs[i] < min_log_hz){
				mels[i] = (float)((freqs[i] - f_min) / f_sp);
			}else{
